...
const ElementSection = ({ className }: ElementSectionProps) => {
  ...
  useEffect(() => {
    ...
    const initSimulation = () => {
      ...
      const leftWall = Bodies.rectangle(-80, height / 2, 160, height, {
        isStatic: true,
      });
      const rightWall = Bodies.rectangle(width + 80, height / 2, 160, height, {
        isStatic: true,
      });
      const ceiling = Bodies.rectangle(width / 2, -80, width + 320, 160, {
        isStatic: true,
      });

      // Creating elements initially stacked together
      const tags: Matter.Body[] = [];
      const centerX = width / 2;
      const centerY = height / 2 - 100; // Start slightly above center

      for (let i = 1; i <= 19; i++) {
        const texturePath = `/assets/img/shape/elements_1_${i}.svg`;

        const tag = Bodies.rectangle(centerX, centerY, 80, 80, {
          chamfer: { radius: 10 },
  ...
  return (
    <section className={`elements-sec ${className}`}>
      <div className="container-fluid border-0 p-0">
        <div
          className="tags-container relative border-0"
          ref={containerRef}
        ></div>
      </div>
    </section>
